####################################
# author: pcshen
# name: lipidNameConversion.R
# created date: 
# revise date: 2026/03/24
# description: Parses and standardizes lipid names from a SummarizedExperiment object using rgoslin,
#              then annotates each recognized lipid with structural characteristics (FA chain length,
#              unsaturation category), lipid classification (LION ID, LIPID MAPS category), and
#              cross-database IDs (LION, ChEBI via GATOM, LIPID MAPS reaction network, LIPID MAPS,
#              SwissLipids, HMDB, KEGG, PubChem, MetaNetX, PlantFA, LipidBank).
#              Specific ambiguous lipid names (e.g., Cholesterol, Sphingosine, Sphinganine) are
#              pre-mapped to standardized goslin-compatible names before parsing.
#              Returns parseable and non-parseable lipids as separate SummarizedExperiment objects.
# input: lipidExpr (SummarizedExperiment object), exclude (logical; if TRUE, excludes non-parseable lipids from output)
# output: list(parseableLipid, nonParseableLipid, allChar)
#         - parseableLipid: SummarizedExperiment with recognized lipids and full annotation in rowData
#         - nonParseableLipid: SummarizedExperiment with unrecognized lipids (NULL if exclude=TRUE or none exist)
#         - allChar: data frame of all lipid characteristics before column selection
####################################
# rm(list=ls())
# library(SummarizedExperiment)
# library(tidyverse)
# library(data.table)
# source('/media/md1200/analysis/Script/LipidSigv2/web_function/SE.R')
# lipidExpr <- readRDS('/media/md1200/analysis/Project/LipidSigv2/ExpressionSet/untarget_se.rds')
#
# system.time(res <- lipidNameConversion(lipidExpr, exclude=FALSE)) #30.823
# res1 <- res$parseableLipid
# res2 <- res$nonParseableLipid
# #saveRDS(res1, '/media/md1200/analysis/Project/LipidSigv2/id_conversion/lipidNameConversion_example.rds')


lipidNameConversion <- function(lipidExpr, exclude=FALSE){
  col.data <- lipidExpr %>% SummarizedExperiment::colData() %>% as.data.frame()
  exp.mat <- lipidExpr %>% SummarizedExperiment::assay() %>% as.data.frame() %>%
    tibble::rownames_to_column("feature")
  ## Modified specific lipid name
  lipidset <- exp.mat %>%
    dplyr::mutate(mod.feature=ifelse(feature %in% c('Cholesterol', 'cholesterol', 'ST 27:1;O'), 'ST 27:1;O',
                                     ifelse(feature %in% c('Sphingosine', 'sphingosine', 'SPB 18:1;O2', 'sphing-4-enine', 'Sphing-4-enine'), 'SPB 18:1;O2',
                                            ifelse(feature %in% c('Sphinganine', 'sphinganine', 'SPB 18:0;O2', 'Dihydrosphingosine', 'dihydrosphingosine'),
                                                   'SPB 18:0;O2', feature)))) %>%
    dplyr::select(feature, mod.feature)
  
  ## SubFunction1: modifyGoslinResults
  goslin.res <- .modifyGoslinResults(lipidset)
  goslin.char <- goslin.res$goslin.char
  
  if(is.null(goslin.char)){
    nonParseableLipid <- lipidExpr
    return(list(parseableLipid=NULL,
                nonParseableLipid=nonParseableLipid,
                allChar=NULL))
  }
  ...
  #### Return SE ####
  ## Parseable lipids
  rownames(exp.mat) <- NULL
  P.expr <- exp.mat %>% dplyr::filter(feature %in% lipid.char$feature) %>%
    dplyr::arrange(match(feature, lipid.char$feature)) %>%
    tibble::column_to_rownames(var='feature') %>% as.matrix()
  parseableLipid <- 
    SummarizedExperiment::SummarizedExperiment(assays=list(exp_data=P.expr),
                                               colData=col.data,
                                               rowData=S4Vectors::DataFrame(lipid.char),
                                               metadata=S4Vectors::metadata(lipidExpr))
  ## Non-parseable lipids
  NP.lipid <- goslin.res$nonparseable.lipid
  nonParseableLipid <- NULL
  if(!is.null(NP.lipid)){
    NP.expr <- exp.mat %>% 
      dplyr::filter(exp.mat$feature %in% NP.lipid) %>%
      dplyr::arrange(match(feature, NP.lipid)) %>%
      tibble::column_to_rownames(var='feature') %>% as.matrix()
    if(exclude == FALSE){
      nonParseableLipid <- 
        SummarizedExperiment::SummarizedExperiment(
          assays=list(exp_data=NP.expr), colData=col.data,
          rowData=S4Vectors::DataFrame(feature=rownames(NP.expr)), metadata=S4Vectors::metadata(lipidExpr))
    }
  }
  return(list(parseableLipid=parseableLipid,
              nonParseableLipid=nonParseableLipid,
              allChar=lipid.char.all))
...
##### Goslin #####
##################
.modifyGoslinResults <- function(lipidset){
  # Lipid Shorthand Name Parsing and Normalization
  goslin <- suppressMessages(rgoslin::parseLipidNames(lipidset$mod.feature))
  goslin %<>% base::cbind(lipidset) %>%
    dplyr::mutate(Original.Name=feature) %>%
    dplyr::select(-feature, -mod.feature)
  # Subset Non-parseable Lipids
  notParseable <- goslin %>% dplyr::filter(Grammar == 'NOT_PARSEABLE')
  
  if(nrow(goslin) == nrow(notParseable)){
    return(list(goslin.char=NULL,
                nonparseable.lipid=notParseable,
                allChar=NULL))
  }
  ...
  # Remove adduct
  goslin.res %<>%
    dplyr::mutate(Species.Name=ifelse(!is.na(Adduct) & !is.na(Species.Name), 
                  ...
                  Sn.Position.Name=ifelse(!is.na(Adduct) & !is.na(Sn.Position.Name), stringr::str_remove(Sn.Position.Name, '\\[.*'), 
                                          Sn.Position.Name))
  # Subset ST lipids
  ...
  FA.chain <- data.table::rbindlist(list(FA.chain, no.FA.lipid),
                                    use.names=TRUE, fill=TRUE)
  FA.chain %<>% dplyr::select('feature'='Original.Name', class,
                              Lipid.Maps.Category, Species.Name, Molecular.Species.Name,
                              'Structural.Species.Name'='Sn.Position.Name', Level,
                              Mass, Sum.Formula, 'Total.FA'='Total.Chain.Sum', Total.C,
                              Total.DB, Total.OH, 'LCB'='LCB_SN', LCB2, 'LCB.C'='LCB_C',
                              'LCB.DB'='LCB_DB', 'LCB.OH'='LCB_OH', LCB.Bond.Type,
                              'FA1'='FA1_SN', 'FA1.C'='FA1_C', 'FA1.DB'='FA1_DB',
                              'FA1.OH'='FA1_OH', FA1.Bond.Type, 'FA2'='FA2_SN',
                              'FA2.C'='FA2_C', 'FA2.DB'='FA2_DB', 'FA2.OH'='FA2_OH',
                              FA2.Bond.Type, 'FA3'='FA3_SN', 'FA3.C'='FA3_C',
                              'FA3.DB'='FA3_DB', 'FA3.OH'='FA3_OH', FA3.Bond.Type,
                              'FA4'='FA4_SN', 'FA4.C'='FA4_C', 'FA4.DB'='FA4_DB',
                              'FA4.OH'='FA4_OH', FA4.Bond.Type, FA, FA.C, FA.DB, FA.OH, Index) %>%
    dplyr::arrange(Index)
  ...
  # Non-parseable lipids
  if(nrow(notParseable) > 0){
    np.lipid <- notParseable$Original.Name
  }else{
    np.lipid <- NULL
  }
  return(list(goslin.char=goslin.m,
              nonparseable.lipid=np.lipid))
}
...
.goslinST <- function(goslin.res){
  ST <- goslin.res %>%
    ...
    dplyr::select('feature'='Original.Name', class, Lipid.Maps.Category,
                  Species.Name, Molecular.Species.Name,
                  'Structural.Species.Name'='Sn.Position.Name', Level,
                  Mass, Sum.Formula, Index)
  return(ST)
}
...
.goslinSE <- function(goslin.res){
  SE <- goslin.res %>%
    ...
    dplyr::mutate(class=ifelse(Lipid.Maps.Main.Class == 'SE 27:1', 'CE',
                               Lipid.Maps.Main.Class),
                  Total.Chain.Sum=stringr::str_remove(Species.Name,
                                             paste0(Lipid.Maps.Main.Class, '/')),
                  Species.Name=ifelse(Lipid.Maps.Main.Class == 'SE 27:1',
                                      stringr::str_replace(Species.Name, 'SE 27:1\\/', 'CE '),
                                      Species.Name),
                  Molecular.Species.Name=ifelse(Lipid.Maps.Main.Class == 'SE 27:1',
                                                stringr::str_replace(Molecular.Species.Name, 'SE 27:1\\/', 'CE '),
                                                Molecular.Species.Name),
                  Sn.Position.Name=ifelse(Lipid.Maps.Main.Class == 'SE 27:1',
...
.goslinPlasmalogen <- function(goslin.res){
  plasmalogen <- goslin.res %>%
    ...
    dplyr::mutate(class=paste0(Lipid.Maps.Main.Class, ' P-'),
                  Total.DB=Total.DB - 1,
                  Species.Name=ifelse(Total.OH == 0,
                                      paste0(class, Total.C, ':', Total.DB),
                                      ifelse(Total.OH == 1,
                                             paste0(class, Total.C, ':', Total.DB, ';O'),
                                             paste0(class, Total.C, ':', Total.DB, ';O', Total.OH))),
                  Total.Chain.Sum=ifelse(Total.OH == 0,
                                         paste0(Total.C, ':', Total.DB),
                                         ifelse(Total.OH == 1,
                                                paste0(Total.C, ':', Total.DB, ';O'),
...
.goslinOthers <- function(goslin.res){
  others <- goslin.res %>%
    ...
    tidyr::separate(Species.Name, into=c('class', 'Total.Chain.Sum'),
                    sep=' ', remove=F) %>%
    dplyr::mutate(class=ifelse(stringr::str_detect(Total.Chain.Sum, '-'),
                               paste0(class, ' ', stringr::str_sub(Total.Chain.Sum, 1, 2)),
                               class),
                  Total.Chain.Sum=ifelse(stringr::str_detect(Total.Chain.Sum, '-'),
...
.goslinFaChainLong <- function(merge.res){
  FA.chain.long <- merge.res %>%
    dplyr::mutate(SN=ifelse(Level == 'MOLECULAR_SPECIES',
    ...
    dplyr::mutate(SN=ifelse(stringr::str_starts(SN, '\\/'), stringr::str_remove(SN, '\\/'), SN)) %>%
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(label=ifelse(Lipid.Maps.Category == 'SP',
                               stringr::str_replace(label, 'FA1', 'LCB'), label)) %>%
    dplyr::mutate(label=ifelse(Lipid.Maps.Category == 'SP',
                               stringr::str_replace(label, 'FA2', 'FA1'), label)) %>%
    dplyr::mutate(label=ifelse(Lipid.Maps.Category == 'GP'
...
.goslinFaChainSpread <- function(FA.chain.long){
  ...
  FA.chain.sp %<>%
    ...
    dplyr::mutate(Total.OH=ifelse(is.na(OH.t), 0,
                                  ifelse(OH.t == 'O', 1,
...
.goslinLysophospholipid <- function(FA.chain){
  lyso <- FA.chain %>%
    dplyr::mutate(FA1=ifelse(is.na(FA1) & Lipid.Maps.Category == 'GP'
...
.reorderMolecularName <- function(goslin.char){
  separate.chain <- goslin.char %>%
    dplyr::filter(!is.na(Molecular.Species.Name) & stringr::str_detect(Molecular.Species.Name, '_')) %>%
    dplyr::select(Index, Molecular.Species.Name) %>%
    dplyr::mutate(Molecular.Species.Name=ifelse(stringr::str_starts(Molecular.Species.Name, 'PE-N\\(FA '),
                                                stringr::str_replace(Molecular.Species.Name, 'PE-N\\(FA ', 'PE-N\\(FA_'),
                                                Molecular.Species.Name)) %>%
    dplyr::mutate(Molecular.Species.Name=ifelse(stringr::str_detect(Molecular.Species.Name, ' O-| P-'),
                                                stringr::str_replace(Molecular.Species.Name, ' ', '_'),
                                                Molecular.Species.Name)) %>%
    dplyr::mutate(Molecular.Species.Name=ifelse(stringr::str_detect(Molecular.Species.Name, '_O-'),
                                                stringr::str_replace(Molecular.Species.Name, '_O-', '_O- '),
                                                Molecular.Species.Name)) %>%
    dplyr::mutate(Molecular.Species.Name=ifelse(stringr::str_detect(Molecular.Species.Name, '_P-'),
  ...
  reorder.chain <- separate.chain %>%
    ...
    dplyr::mutate(new.chain=ifelse(is.na(Fun), paste0(C, ':', DB),
                                   paste0(C, ':', DB, ';', Fun))) %>%
    dplyr::reframe(chain=paste0(new.chain, collapse='_')) %>%
    dplyr::mutate(new.molecular.name=paste0(class, ' ', chain)) %>%
    dplyr::mutate(new.molecular.name=ifelse(stringr::str_detect(new.molecular.name, ' O- '),
  ...
  reorder.res <- goslin.char %>%
    ...
    dplyr::mutate(Molecular.Species.Name=ifelse(!is.na(new.molecular.name),
                                                new.molecular.name, Molecular.Species.Name)) %>%
...
.lionIdConversion <- function(goslin.char){
  class.sys <- readRDS('www/mapping_table/LIPIDMAPS_classification_system.rds')
  lion.char <- readRDS('www/mapping_table/LION_characteristics.rds')
  lion.db <- readRDS('www/mapping_table/LION_IDmapping.rds')
  lipid2lion <- lion.db %>% dplyr::distinct(name, LION_ID)
  ## Modify Class Name
  goslin.char2 <- goslin.char %>%
    dplyr::mutate(LION.Class=ifelse(class == 'FA', 'FFA',
                                    ifelse(class == 'Cholesterol', 'cholesterol',
                                           ifelse(Lipid.Maps.Category == 'GP' & stringr::str_starts(class, 'LP'),
                                                  stringr::str_sub(class, 2, -1), class))))
  #### Sphingolipids ####
  SP <- goslin.char2 %>%
    dplyr::filter(Lipid.Maps.Category == 'SP') %>%
    dplyr::mutate(LION.abbr=ifelse(Level == 'SPECIES',
...
.faUnsaturationCategory <- function(goslin.char){
  cate <- goslin.char %>%
    ...
    dplyr::mutate(Category1=ifelse(!is.na(FA.DB) & FA.DB < 2,
                                   'FA with less than 2 double bonds',
                                   ifelse(!is.na(FA.DB) & FA.DB >= 2,
                                          'Polyunsaturated fatty acid', NA))) %>%
    dplyr::mutate(Category2=ifelse(!is.na(FA.DB) & FA.DB == 0,
                                   'Saturated fatty acid',
                                   ifelse(!is.na(FA.DB) & FA.DB == 1,
                                          'Monounsaturated fatty acid',
                                          ifelse(!is.na(FA.DB) & FA.DB == 2,
                                                 'FA with 2 double bonds',
...
.faChainLengthCategory <- function(goslin.char){
  cate <- goslin.char %>%
    ...
    dplyr::mutate(Category1=ifelse(!is.na(FA.C) & FA.C <= 18,
                                   'FA with 18 carbons or less',
                                   ifelse(!is.na(FA.C) & FA.C > 18,
                                          'FA with more than 18 carbons', NA))) %>%
    dplyr::mutate(Category2=ifelse(!is.na(FA.C) & FA.C < 13, 'FA with less than 13 carbons',
                                   ifelse(!is.na(FA.C) & FA.C %in% c(13:15), 'FA with 13-15 carbons',
                                          ifelse(!is.na(FA.C) & FA.C %in% c(16:18), 'FA with 16-18 carbons',
                                                 ifelse(!is.na(FA.C) & FA.C %in% c(19:21), 'FA with 19-21 carbons',
                                                        ifelse(!is.na(FA.C) & FA.C %in% c(22:24), 'FA with 22-24 carbons',
                                                               ifelse(!is.na(FA.C) & FA.C > 24, 'FA with more than 24 carbons', NA))))))) %>%
    dplyr::mutate(Category3=ifelse(FA.C >= 2 & FA.C <= 6, 'Short-chain FA',
...
.gatomIdConversion <- function(goslin.char){
  lipid2chebi <- readRDS('www/mapping_table/GATOM_IDmapping.rds')
  lipid2chebi2 <- lipid2chebi %>%
    dplyr::group_by(Species) %>%
    dplyr::summarise(GATOM.chebi=paste0(metabolite, collapse='|'))
  #### Sphingolipids ####
  SP <- goslin.char %>%
    dplyr::filter(Lipid.Maps.Category == 'SP') %>%
    ...
    dplyr::mutate(GATOM.abbr=ifelse(class %in% c('ACer', 'SPB', 'SPBP', 'HexCer',
                                                 'Hex2Cer', 'Hex3Cer', 'SHexCer',
                                                 'SHex2Cer'),
                                    Species.Name, GATOM.abbr))
  #### Lysophospholipids ####
  LPL <- goslin.char %>%
    dplyr::filter(Lipid.Maps.Category == 'GP', stringr::str_starts(class, 'LP')) %>%
  ...
  others <- goslin.char %>%
    ...
    dplyr::mutate(GATOM.abbr=ifelse(Level == 'SPECIES', paste0(class, '(', Total.FA, ')'),
    ...
    dplyr::mutate(GATOM.abbr=stringr::str_replace(GATOM.abbr, ' P-\\(', '\\(P-'))
  #### Map to ChEBI ID ####
  chebi <- data.table::rbindlist(list(SP, LPL, FA, GLO, GLOH, others), use.names=TRUE, fill=TRUE) %>%
    dplyr::left_join(lipid2chebi2, by=c('GATOM.abbr'='Species'))
  #### Selected column ####
  gatom.final <- chebi %>%
    dplyr::select(feature, GATOM.abbr, Index) %>%
    #dplyr::select(feature, GATOM.abbr, GATOM.chebi, Index) %>%
    dplyr::arrange(Index)
...
.lipidmapsNetworkIdConversion <- function(goslin.char){
  lipidmaps.class <- readRDS('www/mapping_table/LIPIDMAPS_reaction_network_node.rds')
  ...
  class.id <- goslin.char %>%
    ...
    dplyr::mutate(id=ifelse(Species.Name == 'SPB 18:1;O2', 'Sphingosine',
                            ifelse(Species.Name == 'SPB 18:0;O2',
                                   'Sphinganine', id))) %>%
...
.idMapping <- function(goslin.char){
  mapping.table <- readRDS('www/mapping_table/ID_mapping_table.rds')
  # Species level
  ...
  id.mapping.res %<>% dplyr::select(feature, Index, 'LIPID.MAPS.ID'='LIPIDMAPS',
                                    'SwissLipids.ID'='SwissLipids',
                                    'HMDB.ID'='HMDB', 'ChEBI.ID'='ChEBI',
                                    'KEGG.ID'='KEGG', 'LipidBank.ID'='LipidBank',
                                    'PubChem.CID'='PubChem','MetaNetX.ID'='MetaNetX',
                                    'PlantFA.ID'='PlantFA')
